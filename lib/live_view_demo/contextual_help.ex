defmodule ContextualHelp do
  ...
  @modules [Access, Enum, Keyword, List, Map, MapSet, Range, Stream, File, IO, Path, Port, StringIO, System, Calendar,
            Agent, Application, Config, DynamicSupervisor, GenEvent, Node, Process, Registry, Task, Collectable, Enumerable,
            Inspect, Protocol, Code, Macro]
  ...
  def add_documentation(command, [%{module: module, func_name: func_name, func_ary: func_ary} | rest], docs) do
    ...
    case docs[%{func_name: "#{module}.#{func_name}", func_ary: func_ary}] do
      nil -> [before, "#{module}.#{func_name}"] ++ add_documentation(remaining_command, rest, docs)
      doc -> [before, {"#{module}.#{func_name}", doc}] ++ add_documentation(remaining_command, rest, docs)
    end
  end
  ...
  def retrive_docs([module | remaining_modules]) do
    ...
    docs = Enum.reduce(list, %{}, fn function, acc ->
      case function do
        {{:function, func_name, func_ary}, _, _header, %{"en" => docs}, _} ->
          {:ok, html_doc, _} = Earmark.as_html(docs)
          [module_name] = Module.split(module)
          Map.put(acc, %{func_name: "#{module_name}.#{func_name}", func_ary: func_ary}, %{header: "#{module_name}.#{func_name}/#{func_ary}", docs: html_doc})
        _ ->
          acc
      end
    end)
